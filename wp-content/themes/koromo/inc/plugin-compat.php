...
add_action( 'after_setup_theme', 'koromo_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function koromo_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'koromo_construct_sidebars' );
}

if ( ! function_exists( 'koromo_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'koromo_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function koromo_woocommerce_start() { ?>
...
if ( ! function_exists( 'koromo_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'koromo_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function koromo_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'koromo_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'koromo_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function koromo_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'koromo_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'koromo_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'koromo_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function koromo_bbpress_css() {
...
if ( ! function_exists( 'koromo_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'koromo_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function koromo_buddypress_css() {
...
if ( ! function_exists( 'koromo_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'koromo_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function koromo_beaver_builder_css() {
