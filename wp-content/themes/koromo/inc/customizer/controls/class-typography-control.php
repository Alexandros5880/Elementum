...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Koromo_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Koromo_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'koromo-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'koromo-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), KOROMO_VERSION, true );
			wp_enqueue_style( 'koromo-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), KOROMO_VERSION );

			wp_enqueue_script( 'koromo-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'koromo-typography-selectWoo' ), KOROMO_VERSION, true );
			wp_enqueue_style( 'koromo-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), KOROMO_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'koromo_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'koromo' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'koromo' );
			$this->json[ 'google_fonts' ] = apply_filters( 'koromo_typography_customize_list', koromo_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = koromo_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'koromo' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'koromo' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'koromo' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'koromo' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="koromo-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="koromo-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="koromo-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="koromo-font-transform">
					<label>
