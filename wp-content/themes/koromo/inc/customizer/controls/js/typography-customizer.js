( function( api ) {
	...
	api.controlConstructor['koromo-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.koromo-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the koromo_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'koromo_get_all_google_fonts_ajax',
								koromo_customize_nonce: koromo_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.koromo-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.koromo-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.koromo-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.koromo-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.koromo-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.koromo-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.koromo-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.koromo-font-variant' );
			...
			control.container.on( 'change', '.koromo-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.koromo-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.koromo-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.koromo-font-family select' ).selectWoo();
	jQuery( '.koromo-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".koromo-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.koromo-font-variant' ).hide();
		}
	});
