...
// Breadcrumbs
function baw_custom_breadcrumbs() {
       
    // Do not display on the homepage
    if (get_theme_mod('baw_home_activate_breadcrumb')and( !is_front_page() || !is_home())) {
       
	?> <div class="breadcrumb" itemprop="breadcrumb"> <!-- breadcrumb --> <?php

	/* === OPTIONS === */
	$text['home']     = "<span class='dashicons dashicons-admin-home'></span>"; // text for the 'Home' link
	...
	$frontpage_id = get_option('page_on_front');
		...
		if ( get_query_var('paged') ) {
			...
			echo __('Page', 'baw') . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
		echo '</li><!-- .breadcrumbs -->';
		echo '</ul>';
